...
* **Wrapper and Router Method:** Utilizes a plugin wrapper (DDMF's Metaplugin, Kushview Element, etc.) and a router plugin (Blue Cat's Connector, etc.) to route the output of a project's stem busses to the appropriate inputs of the YSFX plugin running THS in master mode.
...
You are free to use whichever connection method you would like for THS. Note that not all DAWs support the Sidechain Method, so you may find that you need to use the Wrapper and Router Method instead.
